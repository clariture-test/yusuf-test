    ...
    $.fn.formset = function(opts)
    {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            insertDeleteLink = function(row) {
                if (row.is('TR')) {
                    ...
                    row.children(':last').append('<a id="remove-formset" hidden="true" class="' + options.deleteCssClass +'" href="javascript:void(0)">' + options.deleteText + '</a>');
                } else if (row.is('UL') || row.is('OL')) {
                    // If they're laid out as an ordered/unordered list,
                    // insert an <li> after the last list item:
                    row.append('<li><a id="remove-formset" hidden="true" class="' + options.deleteCssClass + '" href="javascript:void(0)">' + options.deleteText +'</a></li>');
                } else {
                    // Otherwise, just insert the remove button as the
                    // last child element of the form's container:
                    row.append('<a id="remove-formset" hidden="true" class="' + options.deleteCssClass + '" href="javascript:void(0)">' + options.deleteText +'</a>');
                }
        ...
        if ($$.length) {
            ...
            if ($$.attr('tagName') == 'TR') {
                ...
                $$.parent().append('<tr><td colspan="' + numCols + '"><a hidden="true" class="' + options.addCssClass + '" href="javascript:void(0)">' + options.addText + '</a></tr>');
                addButton = $$.parent().find('tr:last a');
                addButton.parents('tr').addClass(options.formCssClass + '-add');
            } else {
                // Otherwise, insert it immediately after the last form:
                $$.filter(':last').after('<a hidden="true" class="' + options.addCssClass + '" href="javascript:void(0)">' + options.addText + '</a>');
